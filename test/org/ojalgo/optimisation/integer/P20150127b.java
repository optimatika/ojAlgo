...
/**
 * A smaller version of P20150127a the number constraints have been greatly reduced and a problematic branch-and-bound
 * node identified.
 *
 * @author apete
 */
public class P20150127b {
    ...
    static public void main(final String[] args) throws Exception {
        ...
        if (result.getState().isSuccess()) {
            ...
            /*
             * if (!result.getState().isSuccess()) throw new Exception("Model should be solvable (e.g. x=20, y=-1)?!");
             * // Get the solution BigDecimal valX = result.get(0); BigDecimal valY = result.get(1); int intX =
             * valX.setScale(0, BigDecimal.ROUND_HALF_UP).intValue(); int intY = valY.setScale(0,
             * BigDecimal.ROUND_HALF_UP).intValue(); System.out.println("x = " + valX + " ~ " + intX);
             * System.out.println("y = " + valY + " ~ " + intY); // Verify solution for (int[] coeff : coefficients) {
             * int value = coeff[0] * intX + coeff[1] * intY; BigDecimal exact =
             * valX.multiply(BigDecimal.valueOf(coeff[0])).add(valY.multiply(BigDecimal.valueOf(coeff[1]))); if (value
             * >= 0) throw new Exception(coeff[0] + "*x + " + coeff[1] + "*y = " + value + " must be negative (exact: "
             * + exact + ")"); } /*
             */
        }
    }
    ...
    static List<int[]> getCoefficients() {
        ...
        // just need i=199
        /*
         * result.add(new int[] { 0, 1 }); for (int i = 0; i < 200; i++) { result.add(new int[] { -1 - (i/20), -i }); }
         */
