...
/**
 * Dense KKT system implementation used for small to medium problems. Assembles the full saddle-point matrix
 * when equalities are present; otherwise uses a Cholesky solve on the SPD Hessian. Preallocates and reuses
 * assembly buffers; grows them if needed.
 */
final class DenseKKTSystem implements KKTSystem {
    ...
    public boolean solve(final MatrixStore<Double> H, final MatrixStore<Double> Aeq, final Access1D<?> rx, final Access1D<?> ry,
        ...
        myRhs.fillAll(0.0);
        for (int i = 0; i < n; i++) {
            myRhs.set(i, rx.doubleValue(i));
        }
        for (int r = 0; r < meq; r++) {
            myRhs.set(n + r, ry.doubleValue(r));
        }
        ...
        for (int i = 0; i < n; i++) {
            outDx.set(i, sol.doubleValue(i));
        }
        if (outY != null) {
            for (int r = 0; r < meq; r++) {
                outY.set(r, sol.doubleValue(n + r));
            }
        }
        return true;
    }
